		...
		try {
			...
			this.world = {
				getBlock: (pos) => {
					...
					const block = chunk.getBlock(new Vec3(Math.floor(pos.x), Math.floor(pos.y + 64), Math.floor(pos.z)));
					block.position = pos;
					return block;
				},
			...
			this.player.client.once("spawn", (data) => {
				this.intervalSim = setInterval(() => {
					...
					const PAIP = new ClientPlayerAuthInputPacket(this.player);
					...
					const OLD_POS_DATA = this.player.playerState.pos.clone();
					...
					const iPacket = new ClientPlayerAuthInputPacket(this.player);
