    ...
    */
	...
	handle(client) {
		client.on("spawn", (spawn) => {
			const Event = new SpawnEvent(spawn, this.player.events);
			Event.call();
		});
		...
		client.on("packet", (packet) => {
			...
			switch (packet.data.name) {
				case "play_status":
					...
					break;
				case "start_game":
					const SGP = new StartGameEvent(packet.data.params, this.player.events);
					SGP.call();
					break;

				case "inventory_content":
					const Event = new InventoryContentEvent(packet.data.params, this.player.events);
					Event.call();
					break;
				case "inventory_transaction":
					console.log(packet);
					const ITEvent = new InventoryTransactionEvent(packet.data.params, this.player.events);
					ITEvent.call();
					break;
				case "inventory_slot":
					const ISEvent = new InventorySlotEvent(packet.data.params, this.player.events);
					ISEvent.call();
					break;
				case "set_score":
					break;
				...
				case "level_event":
					break;
				case "text":
					const textE = new ServerTextEvent(packet.data.params, this.player.events, this.player);
					textE.call();
				case "move_player":
					if (packet.data.params.runtime_id == this.player.runtime_entity_id) {
						//console.log(this.player.runtime_entity_id);
						if (this.player.playerState !== null) {
							const pos = this.player.playerState.pos;
